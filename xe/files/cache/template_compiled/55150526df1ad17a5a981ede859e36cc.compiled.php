...
<!--#Meta:layouts/xe_official_v2/css/layout.css--><?php $__tmp=array('layouts/xe_official_v2/css/layout.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:layouts/xe_official_v2/css/layout@official.css--><?php $__tmp=array('layouts/xe_official_v2/css/layout@official.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
<!--#Meta:layouts/xe_official_v2/css/content.css--><?php $__tmp=array('layouts/xe_official_v2/css/content.css','','','');Context::loadFile($__tmp);unset($__tmp); ?>
...
<?php echo $__Context->layout_info->logo_bg_image ?>
			...
			<ul class="account">
                ...
                <?php if($__Context->is_logged){ ?>
                    <a href="<?php echo getUrl('act','dispMemberLogout') ?>"><img src="/michellu/xe/layouts/xe_official_v2/img/buttonLogout.gif" width="33" height="5" alt="LOGOUT" /></a>
                <?php }else{ ?>
                    <a href="#loginWindow" onclick="jQuery('#loginWindow').css('display','block');" accesskey="L"><img src="/michellu/xe/layouts/xe_official_v2/img/buttonLogin.gif" width="25" height="5" alt="LOGIN" /></a>
                <?php } ?>
                </li>
                <?php if(!$__Context->is_logged){ ?>
				<li class="register"><a href="<?php echo getUrl('act','dispMemberSignUpForm') ?>"><span>Register</span></a></li>
                <?php }else{ ?>
				<li class="profile">
				...
				<!-- class="language" | class="language open" -->
					<em class="<?php echo $__Context->lang_type ?>"><span><?php echo $__Context->lang_supported[$__Context->lang_type] ?></span></em>
...
if($__Context->key!= $__Context->lang_type){ ?>
						<li class="<?php echo $__Context->key ?>"><button type="button" onclick="doChangeLangType('<?php echo $__Context->key ?>');return false;"><span><?php echo $__Context->val ?></span></button></li>
                        <?php };
} ?>
					</ul>
				</li>
...
$__Context->_idx=0;
			...
			<form action="<?php echo getUrl() ?>" method="post" class="search"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" />
				...
				<fieldset>
					...
					<input name="" type="image" src="/michellu/xe/layouts/xe_official_v2/img/buttonSearch.gif" alt="Search" class="inputSearch" />
				</fieldset>
			</form>
		</div>
		<hr />
		<div id="body">
			<div id="content">
				...
				<div class="contentHeader">
					<div class="title">
						...
						<h3><?php echo $__Context->layout_info->title_main ?></h3>
					</div>
					<div class="summary">
			...
			<div class="extension e1">
				<div class="lnb">
					<?php if($__Context->layout_info->right_title){ ?>
                        <h2><?php echo $__Context->layout_info->right_title ?></h2>
					<?php }elseif($__Context->selected_menu){ ?>
                        <h2><?php echo $__Context->selected_menu['text'] ?></h2>
                    <?php } ?>
		...
		<div id="sitemap">
			<div class="extension">
				<div class="section logo">
					...
					<img src="/michellu/xe/layouts/xe_official_v2/img/xeKnee.gif" width="102" height="55" alt="XE&#8482; CLEVER CONTENT MANAGEMENT SYSTEM" />
                    <?php }else{ ?>
...
<?php if(!$__Context->is_logged){ ?>
    ...
    <?php require_once('./classes/xml/XmlJsFilter.class.php');$__xmlFilter=new XmlJsFilter('layouts/xe_official_v2/filter','login.xml');$__xmlFilter->compile(); ?>
    <?php if($__Context->member_config->enable_openid=='Y'){;
require_once('./classes/xml/XmlJsFilter.class.php');$__xmlFilter=new XmlJsFilter('layouts/xe_official_v2/filter','openid_login.xml');$__xmlFilter->compile();
} ?>
    <!-- loginWindow -->
