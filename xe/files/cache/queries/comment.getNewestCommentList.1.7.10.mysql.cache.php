<?php if(!defined('__XE__')) exit();
$query = new Query();
$query->setQueryId("getNewestCommentList");
$query->setAction("select");
$query->setPriority("");
if(isset($args->status)) {
${'status54_argument'} = new ConditionArgument('status', $args->status, 'equal');
${'status54_argument'}->createConditionValue();
if(!${'status54_argument'}->isValid()) return ${'status54_argument'}->getErrorMessage();
} else
${'status54_argument'} = NULL;if(${'status54_argument'} !== null) ${'status54_argument'}->setColumnType('number');
if(isset($args->module_srl)) {
${'module_srl55_argument'} = new ConditionArgument('module_srl', $args->module_srl, 'in');
${'module_srl55_argument'}->checkFilter('number');
${'module_srl55_argument'}->createConditionValue();
if(!${'module_srl55_argument'}->isValid()) return ${'module_srl55_argument'}->getErrorMessage();
} else
${'module_srl55_argument'} = NULL;if(${'module_srl55_argument'} !== null) ${'module_srl55_argument'}->setColumnType('number');
if(isset($args->document_srl)) {
${'document_srl56_argument'} = new ConditionArgument('document_srl', $args->document_srl, 'equal');
${'document_srl56_argument'}->checkFilter('number');
${'document_srl56_argument'}->createConditionValue();
if(!${'document_srl56_argument'}->isValid()) return ${'document_srl56_argument'}->getErrorMessage();
} else
${'document_srl56_argument'} = NULL;if(${'document_srl56_argument'} !== null) ${'document_srl56_argument'}->setColumnType('number');

${'list_count58_argument'} = new Argument('list_count', $args->{'list_count'});
${'list_count58_argument'}->ensureDefaultValue('20');
if(!${'list_count58_argument'}->isValid()) return ${'list_count58_argument'}->getErrorMessage();

${'sort_index57_argument'} = new Argument('sort_index', $args->{'sort_index'});
${'sort_index57_argument'}->ensureDefaultValue('list_order');
if(!${'sort_index57_argument'}->isValid()) return ${'sort_index57_argument'}->getErrorMessage();

$query->setColumns(array(
new StarExpression()
));
$query->setTables(array(
new Table('`xe_comments`', '`comments`')
));
$query->setConditions(array(
new ConditionGroup(array(
new ConditionWithArgument('`status`',$status54_argument,"equal", 'and')
,new ConditionWithArgument('`module_srl`',$module_srl55_argument,"in", 'and')
,new ConditionWithArgument('`document_srl`',$document_srl56_argument,"equal", 'and')))
));
$query->setGroups(array());
$query->setOrder(array(
new OrderByColumn(${'sort_index57_argument'}, "asc")
));
$query->setLimit(new Limit(${'list_count58_argument'}));
return $query; ?>